...
def perform_video_analysis(video_path:str, 
    ...
    if len(detections) == 0:
        print("\033[92mNo periodicity detected!\033[0m\n")
    else:
...
def select_polygon_roi(video_path):
    ...
    if len(points) < 3:
        ...
        print(f"Polygon with {len(points)} points selected.")
        print()
...
class ResultViewer:
    def __init__(self, master, frame_fname_list, residual_fname_list, rotation, roi_mask=None):
        ...
        if roi_mask is not None:
            ...
            residual_img = correct_rotation(residual_img, self.rotation)
    ...
    def update_frame(self):
        ...
        # correct rotation if needed
        img_residual = correct_rotation(img_residual, self.rotation)
...
#   python perform_video_analysis_gui.py path/to/video.mp4 --d 5 --space U --epsilon 0.1 --out_folder results/